...
void
GUI_GLFW::UpdateInteface(PROGRAM_MODE CURRENT_MODE, Cell *board, STATUS PLAYER)
{
	mLeftClickf = false;
	mCancelf = false;
	mSpacePressf = false;
...
void
GUI_GLFW::CursorPositionCallback(GLFWwindow *window, double xpos, double ypos)
{
	mX = xpos, mY = ypos;
	//std::cout << " (" << xpos << ", " << ypos << ")\n";
}
...
void
GUI_GLFW::MouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
...
void
GUI_GLFW::KeyboardKeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
...
void
GUI_GLFW::DrawBoard(PROGRAM_MODE CURRENT_MODE, Cell *board)
{
	switch (CURRENT_MODE)
	{
	case STANDBY:
...
void
GUI_GLFW::DrawCell(int i, const Cell &c)
{
	int quad = i / 6;
...
void
GUI_GLFW::DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3)
{
	float vertices[] =
		{
			x1, y1, 0.0,
...
void
GUI_GLFW::DrawTriangle(Point p1, Point p2, Point p3)
{
	DrawTriangle(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
}

void
GUI_GLFW::DrawRectangle(int x1, int y1, int x2, int y2)
{
	float vertices[] =
		{
			x1,
...
void
GUI_GLFW::DrawRectangle(Point p1, Point p2)
{
	DrawRectangle(p1.x, p1.y, p2.x, p2.y);
}

void
GUI_GLFW::DrawLine(int x1, int y1, int x2, int y2)
{
	float vertices[] =
		{
			x1,
...
void
GUI_GLFW::DrawLine(Point p1, Point p2)
{
	DrawLine(p1.x, p1.y, p2.x, p2.y);
}

void
GUI_GLFW::DrawCircle(int x, int y, int radius, bool color)
{
	if (color)
		SetColor("#000000");
	else
...
void
GUI_GLFW::DrawCircle(Point p, int radius, bool color)
{
	DrawCircle(p.x, p.y, radius, color);
}

void
GUI_GLFW::SetColor(int r, int g, int b, bool background)
{
	if (background)
...
void
GUI_GLFW::SetColor(Color c, bool background)
{
	SetColor(c.r, c.g, c.b, background);
}

float
GUI_GLFW::TriangleArea(int x1, int y1, int x2, int y2, int x3, int y3)
{
	return abs((x1 * (y2 - y3) + x2 * (y3 - y1) + x3 * (y1 - y2)) / 2.0);
}

float
GUI_GLFW::TriangleArea(Point p1, Point p2, Point p3)
{
